...
- **Discographies** load live from MusicBrainz; **photos** from Wikipedia; **30-second preview clips** prefer Apple (with Deezer as the fallback), artist-verified before playing; **Apple Music links** via Apple's iTunes lookup. A small gold **waveform** in the now-playing pill pulses to the track's real audio.
...
- A **3D view** (`#/<genre>/holo`) renders the same constellation in real WebGL: drag to orbit, scroll or pinch to zoom, every name always legible, and tapping a star lights its gold relationships and opens the same card, preview, and Rooms as the other views. Nodes are soft, lit "velvet" spheres rather than flat discs. It lazy-loads three.js only when entered, so the main site stays dependency-free; each genre keeps its own backdrop colour.
- **Your selection follows you across views** — pick an artist in the globe, the chord web, or 3D, switch views, and they stay selected (anchored with lit ties in chord; card open in globe and 3D) so you can keep digging without re-finding them.
